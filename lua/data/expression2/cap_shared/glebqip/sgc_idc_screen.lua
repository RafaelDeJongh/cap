# Version 1.5
# Author glebqip(RUS)
# Created 30.11.13 Updated 20.12.13
...
@inputs Start DPL W:wirelink SG:wirelink IC:wirelink ReceivedCode Key KeyUser:entity NewCol IDC:table IrisDP
...
if(~W&->W){reset()}
if(first()|dupefinished()|clk("shutdown1")){W:egpClear() Loaded1=0 timer("ircheck",300)}
...
W:egpText(286,IDC[ReceivedCode1,array][2,string],vec2(380,228)) W:egpColor(286,vec(28,78,83)) W:egpAlign(286,1,1) W:egpFont(286,"Marlett",26)
...
if(Loaded==1){
if($Inbound&Inbound){W:entity():soundPlay(122,200,"alexalx/glebqip/idc_incomming.wav") ReceivedCode1=0 Iris1=1}
if(($Inbound&!Inbound)|clk("Loaded")){ReceivedCode1=0 GDOStatus=0 Iris1=0 IDCStatus=0 GDOText=""}
if(~ReceivedCode&ReceivedCode1!=ReceivedCode&(ReceivedCode>0&Inbound)){ReceivedCode1=ReceivedCode}
if(~Key&Key==9&((KeyUser==owner()&->KeyUser)|(!->KeyUser))){RB++ timer("RBTR",200) if(RB==2){timer("shutdown",1) Loaded=-1 RB=0 }}
...
if($Inbound&!ReceivedCode&Inbound){W:egpText(276,"INCOMMING TRAVELER",vec2(380,167)) W:egpSize(276,21)
...
if((($Inbound|~ReceivedCode)&(!ReceivedCode&!Inbound)|(~ReceivedCode&ReceivedCode>0))|(clk("Loaded"))){for(I=0,7){AA=0 W:egpAlpha(273+I,0)}}
if((($Inbound|~ReceivedCode)&(ReceivedCode|Inbound))){for(I=0,7){W:egpAlpha(273+I,255)}}
...
if(clk("lines")&E1==1){IDCStatus=1 W:entity():soundPlay(122,200,"alexalx/glebqip/idc_start.wav") W:egpSetText(268,"INCOMMING") W:egpSetText(269,"SIGNAL")}
if(~ReceivedCode&ReceivedCode&E1<2){timer("lines",1) GDOText="WAIT" GDOStatus=2 timer("STAT",2000)} if(clk("lines")&E1<99&ReceivedCode){E1++ W:egpLine(117+E1,vec2(251+(E1)*2.5425,321+E[E1,number]),vec2(251+(E1+1)*2.5425,321+E[E1+1,number])) W:egpColor(117+E1,vec(255,100,100)) timer("lines",1)}
#if(clk("STAT")){GDOStatus=-1}
if($E1&E1>=98&E1<=100){timer("lines2",100)}
if(clk("lines2")&E1>=99&E1<199&ReceivedCode){E1++ W:egpPos(354,vec2(258+(E1-99)*2.5425,280)) W:egpColor(117+E1-99,vec(255,0,0)) if(E1==198){W:egpAlpha(354,0) AA=1 W:egpSetText(268,"DECODING") W:egpSetText(269,"SIGNAL")} timer("lines2",1)}
if(~ReceivedCode&!ReceivedCode|clk("Loaded")){for(I=0,99){W:egpRemove(117+I) E1=0}}
if(changed(AA)&AA){for(I=0,99){W:egpRemove(117+I)}}
if($AA&AA){timer("cifri",50) for(I=288,303){W:egpAlpha(I,255)}} if(($AA&!AA)|(clk("Loaded"))|($Inbound&!Inbound)|(~ReceivedCode&ReceivedCode>0)){W:egpSetText(268,"") W:egpSetText(269,"") W:egpAlpha(354,0) EndSymb="" EndSymb2="" CodeStat=0 for(I=288,303){W:egpAlpha(I,0)} for(I=0,13){W:egpSetText(304+I,"")}}
if($CodeStat&!CodeStat|clk("Loaded")){for(I=0,6){W:egpAlpha(281+I,0)}}
if($CodeStat&CodeStat==1){GDOText="ACCEPT" W:entity():soundPlay(122,200,"alexalx/glebqip/idc_accept.wav") GDOStatus=-1 Iris1=0 IDCStatus=3 IDCName=IDC[ReceivedCode1,array][2,string]
...
W:egpText(286,IDC[ReceivedCode1,array][2,string],vec2(380,228))
W:egpAlpha(287,255)}
if($CodeStat&CodeStat==2){GDOText="DENIED" W:entity():soundPlay(122,200,"alexalx/glebqip/idc_error.wav") GDOStatus=-1 Iris1=1 IDCStatus=5
...
W:egpAlpha(287,255)}
if($CodeStat&CodeStat==3){GDOText="EXPIRED" W:entity():soundPlay(122,200,"alexalx/glebqip/idc_error.wav") GDOStatus=-1 Iris1=1 IDCStatus=4 IDCName=IDC[ReceivedCode1,array][2,string]
W:egpAlpha(281,255) W:egpColor(281,vec(200,100,0))
W:egpAlpha(282,255)
W:egpAlpha(283,255)
W:egpAlpha(286,255)
W:egpSetText(283,"WARNING") W:egpColor(283,vec(200,100,0)) W:egpSize(283,58)
for(I=0,1){W:egpAlpha(284+I,255) W:egpText(284+I,RSR3[I+1,string],vec2(380,167+I*24))}
W:egpText(286,IDC[ReceivedCode1,array][2,string],vec2(380,228))
W:egpAlpha(287,255)}
#
if(clk("kvadratiki")){if(!Active){for(I=318,351){if(random()>0.5){Alpha1=255}else{Alpha1=0} W:egpAlpha(I,Alpha1)}} timer("kvadratiki",150)}
...
if(~ReceivedCode&ReceivedCode){for(I=0,13){if(I+1<=ReceivedCode1:toString():length()){F[I+1,string]=ReceivedCode1:toString()[I+1]}else{F[I+1,string]="X"}}}
...
if(~IrisDP&IrisDP&Iris1&!Irl){Iris1=0 Irl=1 timer("unl",50)}
if(~IrisDP&IrisDP&!Iris1&!Irl){Iris1=1 Irl=1 timer("unl",50)}
if(clk("unl")){Irl=0}
}
}
else
{
if(clk("ircheck")){
if(!Iris){SG:stargateIrisToggle()}#IrisControl=1 timer("irtg",100)}
timer("ircheck",150)}
