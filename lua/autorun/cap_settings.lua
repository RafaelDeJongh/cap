...
local function SG_Settings_OpenNet()
	...
	/*CapPanel = vgui.Create( "Panel" )
	--CapPanel:SetPos( 50, 50 )
	--CapPanel:SetMinimumSize( 350, 400 )
	CapPanel:SetSize( sizew+10, sizeh+50 )
	/*CapPanel:SetTitle( "Testing Derma Stuff" )
	CapPanel:SetVisible( true )
	CapPanel:SetDraggable( true )
	CapPanel:SetSizable( true ) *
	CapPanel:Center();
	--CapPanel:ShowCloseButton( true )
	...
	local lframe = vgui.Create( "Panel" , AdminFrame);
	lframe:SetPos(10, 10);
	lframe:SetSize(190, 15);
	...
	/*if (SGGetConvar("stargate_has_rd")!=1) then
	...
	GroupConvarFrame.Paint = function(self)
		...
		if (SGGetConvar("stargate_has_rd")!=1) then
			draw.RoundedBox( bor, 130-diff, 335-diff, 340+2*diff, 50+2*diff, col);
			draw.RoundedBox( bor, 130, 335, 340, 50, col2);
		end
	...
	for k,val in pairs(sgsettings) do
		...
		if ((k==2 or k==3 or k==4) and SGGetConvar("stargate_has_rd")!=1) then
	...
	for k,val in pairs(dhdsettings) do
		...
		if (k==3 and SGGetConvar("stargate_has_rd")!=1) then
			box:SetDisabled(true);
			box:SetTextColor(Color(128,128,128));
			box:SetValue(0);
		end
	...
	MiscFrame.Paint = function(self)
		...
		draw.RoundedBox( bor, 5-diff, 5-diff, 230+2*diff, 330+2*diff, col);
		draw.RoundedBox( bor, 5, 5, 230, 330, col2);

		draw.RoundedBox( bor, 5-diff, 350-diff, 230+2*diff, 65+2*diff, col);
		draw.RoundedBox( bor, 5, 350, 230, 65, col2);
	...
	CapConvarFrame.Paint = function(self)
		...
		draw.RoundedBox( bor, 5-diff, 20-diff, 280+2*diff, sizeh-60+2*diff, col);
		draw.RoundedBox( bor, 5, 20, 280, sizeh-60, col2);

		draw.RoundedBox( bor, 300-diff, 20-diff, 280+2*diff, sizeh-60+2*diff, col);
		draw.RoundedBox( bor, 300, 20, 280, sizeh-60, col2);
	...
	DVScrollBarFrame:SetPos(10, 20);
	DVScrollBarFrame:SetSize(250,sizeh-60);
	...
	DVScrollBar:SetSize( 16, sizeh-70)
	DVScrollBar:SetPos(265,25)
	...
	DVScrollBar.Think = function() for k,s in pairs(sliders) do s:SetPos(5, DVScrollBar:GetOffset()+8+40*k) end end
	DVScrollBar:SetUp(40,20+40*(i-9))
	...
	local DVScrollBarFrame = vgui.Create("DFrame", CapConvarFrame);
	DVScrollBarFrame:SetPos(305, 20);
	DVScrollBarFrame:SetSize(250,sizeh-60);
	DVScrollBarFrame:SetTitle("");
	DVScrollBarFrame:ShowCloseButton(false);
	DVScrollBarFrame:SetVisible(true);
	...
	local img = vgui.Create("DImage", GroupConvarFrame)
	img:SetPos(428,145);
	img:SetSize(140,140);
	...
	local SheetItemTwo = vgui.Create( "DLabel" , CategoryContentTwo )
	SheetItemTwo:SetText( "Coming soon..." )
	...
	--PropertySheet:AddSheet( SGLanguage.GetMessage("stargate_menu_t4"), SheetItemTwo, "icon16/wrench.png", false, false, "bla bla bla" )
...
local function NetSGSettings( len )
	local count = net.ReadInt(16);
	for i=1,count do
		sg_convars[net.ReadString()] = net.ReadInt(16);
	end
	SG_Settings_OpenNet();
end
net.Receive("_sg_convars", NetSGSettings);
