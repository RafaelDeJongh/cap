...
function StarGate_Settings(Panel)
	...
	if (LocalPlayer():IsAdmin()) then
	    ...
	    convarsmenu:SetText(Language.GetMessage("stargate_settings_01"));
		...
		local convarsmenu = vgui.Create("DButton", Panel);
	    convarsmenu:SetText(Language.GetMessage("stargate_settings_02"));
	    convarsmenu:SetSize(150, 25);
		convarsmenu.DoClick = function ( btn )
	...
	Panel:Help("");
	Panel:Help(Language.GetMessage("stargate_settings_06")):SetTextColor(DGREEN);
	local clientlang = vgui.Create("DMultiChoice",Panel);
	clientlang:SetSize(50,20);
	clientlang:SetText(Language.GetClientLanguage());
	clientlang.TextEntry:SetTooltip(Language.GetMessage("stargate_settings_07"));
	clientlang.TextEntry.OnTextChanged = function(TextEntry)
		...
		if (letters!="") then Language.SetClientLanguage(letters); end
	end
	clientlang.OnSelect = function(panel,index,value)
		if (value!="") then Language.SetClientLanguage(value); end
	end
	...
	Panel:Help(Language.GetMessage("stargate_settings_03")):SetTextColor(DGREEN);
	Panel:Help(langstext.."."):SetTextColor(DGREEN);
	Panel:Help(Language.GetMessage("stargate_settings_04"));
	Panel:Help("");
	Panel:Help(Language.GetMessage("stargate_settings_05"));
	Panel:Help("");
	Panel:Help(Language.GetMessage("stargate_settings_08"));
