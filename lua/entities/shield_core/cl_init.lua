...
function VGUI:Init()
	...
	local DermaPanelInfo = vgui.Create( "DFrame" )
   	DermaPanelInfo:SetPos(ScrW()/10+450, ScrH()/10)
   	DermaPanelInfo:SetSize( 310, 160 )
	DermaPanelInfo:SetTitle( "Shield Core Info" )
   	DermaPanelInfo:SetVisible( true )
   	DermaPanelInfo:SetDraggable( false )
   	DermaPanelInfo:ShowCloseButton( true )
   	DermaPanelInfo:MakePopup()
	DermaPanelInfo.Paint = function()
		...
		surface.DrawOutlinedRect( 0, 0, DermaPanelInfo:GetWide(), DermaPanelInfo:GetTall() )

    end

	local image2 = vgui.Create("TGAImage" , DermaPanelInfo);
    image2:SetSize(10, 10);
    image2:SetPos(10, 10);
    image2:LoadTGAImage("materials/gui/cap_logo.tga", "MOD");

	local back = vgui.Create("DPanel", DermaPanelInfo);
	back:SetPos(15, 25);
	back:SetSize(280, 120);
	back.Paint = function() draw.RoundedBox( 6, 0, 0, 280, 120, Color( 170, 170, 170, 255)); end

	local back3 = vgui.Create("DPanel", DermaPanelInfo);
	back3:SetPos(20, 30);
	back3:SetSize(270, 110);
	back3.Paint = function() draw.RoundedBox( 6, 0, 0, 270, 110, Color( 100, 100, 100, 255)); end

	local text1 = vgui.Create("DLabel", DermaPanelInfo);
	text1:SetPos( 60, 40);
	text1:SetText("HL2 weapons doesnt shoot shield?");
	text1:SizeToContents();

	local text2 = vgui.Create("DLabel", DermaPanelInfo);
	text2:SetPos( 80, 60);
	text2:SetText("Get their LUA replacements!");
	text2:SizeToContents();

	local text3 = vgui.Create("DLabel", DermaPanelInfo);
	text3:SetPos( 120, 80);
	text3:SetText("More info at:");
	text3:SizeToContents();

	local text4 = vgui.Create("DTextEntry", DermaPanelInfo);
	text4:SetPos( 30, 110);
	text4:SetSize(250, 20);
	text4:SetText("www.facepunch.com/showthread.php?t=829230");
	text4.OnEnter = function() end
