...
function ENT:PreEntityCopy()
	...
	if self.ConTable then
		dupeInfo.ConTable = self.ConTable;
	end

	duplicator.StoreEntityModifier(self, "SGGravContr", dupeInfo)
	StarGate.WireRD.PreEntityCopy(self)
end

function ENT:PostEntityPaste(ply, Ent, CreatedEntities)

	local dupeInfo = Ent.EntityMods["SGGravContr"] or {}

	if dupeInfo.ConTable then
		self.ConTable = dupeInfo.ConTable
	end
